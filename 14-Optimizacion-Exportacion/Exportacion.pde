...
//Variables globales
  ...
  //Imagenes
  PImage umbreon,fondo,icono; // Creamos nuestros objetos de imagen
...
void settings(){
  size(600, 600); //Tamaño de ventana y entorno (Tamaño x, Tamaño y, Entorno)
                       //Recuerden que si ponemos, P2D activamos OPENGL
                       //Se recomienda siempre usar con PGraphics
  smooth(); //Suavisa las lineas
...
void setup() {
  ...
  surface.setTitle("Pokemon de Shini"); // Define el titulo de la ventana. 
  icono =loadImage("icon/icono.png");   // Cargamos la imagen del icono.
  surface.setIcon(icono);               // Definimos el icono de la ventana.
  surface.setResizable(false);          // Definimos que no se puede redimensionar la ventana.
                                        // Por defecto ya viene en false, con true lo permiten.

  //window.setResizable(true);          // Misma funcion que surface.
  //windowTitle("Pokemon de Shini")     // Misma funcion que surface                       
  //fullScreen();                       // Inicia en pantalla comprleta
